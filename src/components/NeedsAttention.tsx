...
const kindColor = {
  ...
  task_due_soon: 'text-warning-600 bg-warning-100',
  unconfirmed_booking: 'text-warning-600 bg-warning-100',
  outstanding_payment: 'text-info-600 bg-info-100',
  inactive_customer: 'text-surface-600 bg-surface-100',
  lead_unattended: 'text-[#6a5ce0] bg-[#ede9fe]',
}
          ...
          {items.map((item) => {
            ...
            return (
              <li key={item.id}>
                <Link
                  to={item.link}
                  className="group flex items-center gap-3 rounded-[12px] bg-white p-3 shadow-soft transition-shadow hover:shadow-soft-md"
                >
                  <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-[8px] ${kindColor[item.kind]}`}>
